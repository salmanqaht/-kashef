...
st.markdown("""
<style>
html, body,.stApp, [data-testid="stAppViewContainer"]{background:#0B1220!important; background-color:#0B1220!important}
[data-testid="stHeader"]{background:#0B1220!important}
.card{background:#151E32; border:1px solid #2A3A5C; border-radius:16px; padding:14px; margin:8px 0; text-align:center}
...
div.stButton>button{background:#22c55e!important; color:white!important; height:56px; width:100%; font-weight:900; border-radius:12px; font-size:19px; border:none!important}
</style>
""", unsafe_allow_html=True)

st.markdown("<h1 style='text-align:center; color:white'>🛡️ كاشف V10 Final</h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align:center; color:#94a3b8'>Black Edition - جاهز للمسابقة</p>", unsafe_allow_html=True)
...
def check(u):
    ...
    if "bit.ly" in d: s+=22; logs.append(("✂️ رابط مختصر","يخفي الوجهة",22))
...
if st.button("افحص الآن 🔍"):
    ...
    if not target:
        ...
        elif score>=35: color="#f59e0b"; level="مشبوه ⚠️ انتبه"
        ...
        for t,v,p in logs:
            st.markdown(f'<div class="card" style="display:flex; justify-content:space-between"><span style="color:white">{t} - {v}</span><b style="color:#f59e0b">+{p}</b></div>', unsafe_allow_html=True)

st.caption("V10 Final Black | مكة 2026")
